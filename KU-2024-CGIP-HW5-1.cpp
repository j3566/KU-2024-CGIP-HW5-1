...
void display() {
	...
	for (const auto& triangle : gTriangles) {
		for (int i = 0; i < 3; ++i) {
			...
			glNormal3f(gNormals[index].x, gNormals[index].y, gNormals[index].z);
			glVertex3f(gPositions[index].x, gPositions[index].y, gPositions[index].z);
		}
	}
	glEnd();
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("HW5 Immediate Mode");
